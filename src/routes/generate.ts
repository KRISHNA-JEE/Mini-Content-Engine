...
generateRouter.post("/generate", upload.single("productImage"), async (req, res) => {
  ...
  if (process.env.VERCEL === "1") {
    await processJob(jobId, productName, description);
    const result = await pool.query<Job>("SELECT * FROM jobs WHERE id = $1", [jobId]);
    if (result.rows[0]) {
      res.status(200).json(serializeJob(result.rows[0]));
      return;
    }
    res.status(500).json({ error: "Job processing finished but the job row could not be loaded" });
    return;
  }
